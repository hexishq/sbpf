...
/// (`region.redirect(...)` in 0.23). The inline tables are snapshotted once per execution, so a
/// region that moves afterwards would leave the fast path reading freed/stale host memory.
/// Reachable only when the moving region is ALONE in its address-space slot.
#[test]
fn attack_region_host_addr_moves_mid_execution() {
    ...
    let run = |inline: bool| -> u64 {
        ...
        // the violation handler flips it to `after` (the CoW unshare). Capture a raw pointer so
        // the handler stays `Fn` (`AccessViolationHandler` is not `FnMut`).
        ...
        let after_ptr: *mut [u8] = &raw mut after[..];
        ...
        let (_count, result) = vm.execute_program(
            &executable,
            &mut ExecutionMode::Jit,
            &mut call_frames,
        );
        match result {
            ProgramResult::Ok(v) => v,
            ProgramResult::Err(e) => panic!("inline={} failed: {:?}", inline, e),
        }
    };
